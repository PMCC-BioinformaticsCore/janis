...
class VarDict(CommandTool):
    ...
    vardict_inputs = [
        ToolInput("indels3prime", Boolean(optional=True), prefix="-3", position=1, shell_quote=False,
                  doc="Indicate to move indels to 3-prime if alternative alignment can be achieved."),
        ToolInput("amplicon", Float(optional=True), prefix="-a", position=1, shell_quote=False,
        ...
        ToolInput("minReads", Int(optional=True), prefix="-B", position=1, shell_quote=False,
                  doc="The minimum # of reads to determine strand bias, default 2"),
        ToolInput("chromNamesAreNumbers", Boolean(optional=True), prefix="-C", position=1, shell_quote=False,
                  doc="Indicate the chromosome names are just numbers, such as 1, 2, not chr1, chr2"),
        ToolInput("chromColumn", Int(optional=True), prefix="-c", position=1, shell_quote=False,
                  doc="The column for chromosome"),
        ToolInput("debug", Boolean(optional=True), prefix="-D", position=1, shell_quote=False,
                  doc="Debug mode.  Will print some error messages and append full genotype at the end."),
        ToolInput("splitDelimeter", String(optional=True), prefix="-d", position=1, shell_quote=False,
                  doc="The delimiter for split region_info, default to tab \"\t\""),
        ToolInput("geneEndCol", Int(optional=True), prefix="-E", position=1, shell_quote=False,
                  doc="The column for region end, e.g. gene end"),
        ToolInput("segEndCol", Int(optional=True), prefix="-e", position=1, shell_quote=False,
        ...
        ToolInput("printHeaderRow", Boolean(optional=True), prefix="-h", position=1, shell_quote=False,
                  doc="Print a header row describing columns"),
        ToolInput("indelSize", Int(optional=True), prefix="-I", position=1, shell_quote=False,
                  doc="The indel size.  Default: 120bp"),
        ToolInput("outputSplice", Boolean(optional=True), prefix="-i", position=1, shell_quote=False,
                  doc="Output splicing read counts"),
        ToolInput("performLocalRealignment", Int(optional=True), prefix="-k", position=1, shell_quote=False,
                  doc="Indicate whether to perform local realignment.  Default: 1.  Set to 0 to disable it. "
                      "For Ion or PacBio, 0 is recommended."),
        ToolInput("minMatches", Int(optional=True), prefix="-M", position=1, shell_quote=False,
        ...
        ToolInput("maxMismatches", Int(optional=True), prefix="-m", position=1, shell_quote=False,
        ...
        ToolInput("regexSampleName", String(optional=True), prefix="-n", position=1, shell_quote=False,
                  doc="The regular expression to extract sample name from BAM filenames. "
                      "Default to: /([^\/\._]+?)_[^\/]*.bam/"),
        ToolInput("mapq", String(optional=True), prefix="-O", position=1, shell_quote=False,
                  doc="The reads should have at least mean MapQ to be considered a valid variant. "
                      "Default: no filtering"),
        ToolInput("qratio", Float(optional=True), prefix="-o", position=1, shell_quote=False,
                  doc="The Qratio of (good_quality_reads)/(bad_quality_reads+0.5). "
                      "The quality is defined by -q option.  Default: 1.5"),
        ToolInput("readPosition", Float(optional=True), prefix="-P", position=1, shell_quote=False,
        ...
        ToolInput("pileup", Boolean(optional=True), prefix="-p", position=1, shell_quote=False,
                  doc="Do pileup regardless of the frequency"),
        ToolInput("minMappingQual", Int(optional=True), prefix="-Q", position=1, shell_quote=False,
                  doc="If set, reads with mapping quality less than INT will be filtered and ignored"),
        ToolInput("phredScore", Int(optional=True), prefix="-q", position=1, shell_quote=False,
        ...
        ToolInput("region", String(optional=True), prefix="-R", position=1, shell_quote=False,
                  doc="The region of interest.  In the format of chr:start-end.  If end is omitted, "
                      "then a single position.  No BED is needed."),
        ToolInput("minVariantReads", Int(optional=True), prefix="-r", position=1, shell_quote=False,
                  doc="The minimum # of variant reads, default 2"),
        ToolInput("regStartCol", Int(optional=True), prefix="-S", position=1, shell_quote=False,
                  doc="The column for region start, e.g. gene start"),
        ToolInput("segStartCol", Int(optional=True), prefix="-s", position=1, shell_quote=False,
                  doc="The column for segment starts in the region, e.g. exon starts"),
        ToolInput("minReadsBeforeTrim", Int(optional=True), prefix="-T", position=1, shell_quote=False,
        ...
        ToolInput("freq", Int(optional=True), prefix="-V", position=1, shell_quote=False,
                  doc="The lowest frequency in the normal sample allowed for a putative somatic mutation. "
                      "Defaults to 0.05"),
        ToolInput("vcfFormat", Boolean(optional=True), prefix="-v", position=1, shell_quote=False,
                  doc="VCF format output"),
        ToolInput("vs", String(optional=True), prefix="-VS", position=1, shell_quote=False,
                  doc="[STRICT | LENIENT | SILENT] How strict to be when reading a SAM or BAM: "
                      ...
                      "Default: LENIENT"),
        ToolInput("bp", Int(optional=True), prefix="-X", position=1, shell_quote=False,
        ...
        ToolInput("zeroBasedCoords", Int(optional=True), prefix="-z", position=1, shell_quote=False,
