...
def prepare_code_tool_page(tool: CodeTool, versions: List[str]):
    ...
    required_input_tuples = [
        [i.id(), i.intype.id(), i.doc] for i in tool.inputs() if not i.intype.optional
    ]
    optional_input_tuples = [
        [i.id(), i.intype.id(), i.doc] for i in tool.inputs() if i.intype.optional
    ]
...
:Required inputs:
{(2 * nl).join(f"   - ``{ins.id()}: {ins.intype.id()}``" for ins in tool.inputs() if (not ins.intype.optional and ins.default is None))}
:Outputs: 
{(2 * nl).join(f"   - ``{out.id()}: {out.outtype.id()}``" for out in tool.outputs())}
