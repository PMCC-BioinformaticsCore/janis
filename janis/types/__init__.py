...
from janis.utils.errors import TooManyArgsException, IncorrectArgsException, InvalidByProductException, \
...
class InputSelector(Selector):
    ...
    def to_string_formatter(self):
        kwarg = {self.input_to_select: self.input_to_select}
...
class StringFormatter(Selector):
    def __init__(self, format, **kwargs):
        ...
        if balance > 0:
            Logger.warn("There was an imbalance of braces in the string _format, this might cause issues with concatenation")
        ...
        if not keywords == skwargs:
            ...
            if not keywords.issubset(skwargs):
                raise IncorrectArgsException("The _format required additional arguments to be provided by "
                                           "**kwargs, requires the keys:" + ", ".join(keywords - skwargs))
            else:
                raise TooManyArgsException("The **kwargs contained unrecognised keys: "
    ...
    def resolve_with_resolved_values(self, **resolved_values):
        ...
        if s1 != actual_keys:
            ...
            raise Exception("The format for the string builder has changed since runtime, or an internal error has"
                            " occurred. The following keys did not appear in both sets: " + ", ".join(diff))

        s2 = set(resolved_values.keys())

        missing_keys = s1 - s2
        if len(missing_keys) > 0:
            raise IncorrectArgsException("There were missing parameters when formatting string: "
                                         + ", ".join(missing_keys))

        unresolved_values = [f"{r} ({type(resolved_values[r]).__name__})" for r in resolved_values
                             if not any(isinstance(resolved_values[r], t) for t in StringFormatter.resolved_types)]
        if len(unresolved_values) > 0:
            raise ValueError("There were unresolved parameters when formatting string: " + ", ".join(unresolved_values))
    ...
    def __add__(self, other):
        if isinstance(other, str):
            ...
            return self._create_new_formatter_from_strings_and_args([self._format, other], **self.kwargs)
            ...
            if len(intersection) > 0:
                not_same_args = [k for k in intersection if self.kwargs[k] != other.kwargs[k]]
            ...
            return StringFormatter._create_new_formatter_from_strings_and_args([self._format, other._format], **new_args)
    ...
    def _create_new_formatter_from_strings_and_args(strings: [str], **kwargs):
        ...
        try:
            ...
            new_params = set(get_keywords_between_braces(new_format)[0] - set(kwargs.keys()))
            raise InvalidByProductException("Joining the input files (to '{new_format}') created the new params: " +
                                            ', '.join(new_params))
