...
class TestWehi(TestCase):
    ...
    def test_parallel(self, mute=False):
        if mute:
            Logger.mute()
        ...
        w = Wehi("Parallel")
        w.parse_string(_parallel_branch)
        w.workflow.draw_graph()
        Logger.unmute()
...
inputs:
    ...
    fastqFile:
        type: Fastq
        forward: forwardFastq
        back: backwardFastq
        
    reference:
        type: FastaWithDict # ".fai", ".amb", ".ann", ".bwt", ".pac", ".sa", "^.dict"
        path: /path/to/reference.fasta"
    
    knownSites:
        type: VCFIDX
        path: /path/to/known_sites.vcf
    
    readGroupHeaderLine: "line here"
    
steps:
    s1_alignsortedbam:
        # Nested workflow with BWA + SamtoolsView + SortSam
        tool: alignsortedbam
        fastq: fastqFile
        reference: reference
        read_group_header_line: readGroupHeaderLine
        tmpdir: tmpdir
        
    s3_markdup:
        tool: gatk4markduplicates
        input: s1_alignsortedbam        # automatically match the BamPair
        tmpDir: tmpdir
        
    s4_recal:
        tool: gatk4baserecalibrator
        input: s3_markdup               # Automatically match bampair
        reference: reference
        knownSites: knownSites
        tmpDir: tmpdir
        
outputs:
    o1: s1_alignsortedbam/o3_sortsam
